...
void hra() {
	...
	string underscore(randomword.length(), '_');  //nova premennı typu string ktorej hodnota je podèiarka a velkost alebo poèet podèiarok závisi podla dlzky premmenej randomword
	...
	while (live > 0) // hra bude bea dokym je ivot viac ako 0
	{
		...
		if (underscore == randomword) // ak sa slovo zhoduje so všetkymi pismenami v underscore premenne tak program skoncí a zaèni od znova len treba poèka 3 sekundy a hra sa od znova 
		{
